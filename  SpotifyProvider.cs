...
using Spotify.NetStandard.Client.Authentication; using Spotify.NetStandard.Client.Interfaces; using Spotify.NetStandard.Requests;
...
public class SpotifyProvider {
    ...
    // Constants
    private const string client_id = "daa629ffab36493a95aa04848746e934";
...
private AccessToken? _token;
    // Private Methods
    private Uri GetCurrentUri() => _navigation.ToAbsoluteUri(_navigation.Uri);
private async Task SetTokenAsync(AccessToken? token) => await _storage.SetItemAsync(nameof(_token), token);
    // Constructor
